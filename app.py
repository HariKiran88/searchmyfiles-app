...
def _configure_tesseract() -> None:
    ...
    if custom_cmd:
        ...
        tessdata_dir = os.path.join(os.path.dirname(custom_cmd), "tessdata")
        if os.path.exists(tessdata_dir) and not os.getenv("TESSDATA_PREFIX"):
            os.environ["TESSDATA_PREFIX"] = tessdata_dir
        return

    scoop_cmd = os.path.expandvars(r"%USERPROFILE%\scoop\shims\tesseract.exe")
    if os.path.exists(scoop_cmd):
        pytesseract.pytesseract.tesseract_cmd = scoop_cmd
        scoop_tessdata = os.path.expandvars(r"%USERPROFILE%\scoop\apps\tesseract\current\tessdata")
        if os.path.exists(scoop_tessdata) and not os.getenv("TESSDATA_PREFIX"):
            os.environ["TESSDATA_PREFIX"] = scoop_tessdata
        return

    common_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
    if os.path.exists(common_cmd):
        pytesseract.pytesseract.tesseract_cmd = common_cmd
        common_tessdata = r"C:\Program Files\Tesseract-OCR\tessdata"
        if os.path.exists(common_tessdata) and not os.getenv("TESSDATA_PREFIX"):
...
def _build_duckduckgo_query(text: str, max_terms: int = 8) -> str:
    ...
    filtered = [tok for tok in tokens if tok not in _WEB_TOPIC_STOPWORDS and not tok.isdigit()]
    if not filtered:
        return cleaned[:140]
...
def _collect_web_sources(query: str, max_results: int = 12) -> Tuple[str, str, str, List[Dict[str, str]]]:
    ...
    try:
        ddgs_sources = _search_ddgs_text_results(query=query, max_results=max_results)
        if ddgs_sources:
    ...
    try:
        result = _search_duckduckgo_instant_answer(query=query, max_results=max_results)
    ...
    if len(sources) < 4:
        fallback = _search_duckduckgo_html_results(query=query, max_results=max_results)
        if fallback:
    ...
    if len(sources) < 4:
        lite = _search_duckduckgo_lite_results(query=query, max_results=max_results)
        if lite:
            seen = {str(s.get("url") or "").strip() for s in sources}
            for item in lite:
                key = str(item.get("url") or "").strip()
    ...
    if len(sources) < 4:
        bing = _search_bing_html_results(query=query, max_results=max_results)
        if bing:
            seen = {str(s.get("url") or "").strip() for s in sources}
            for item in bing:
                key = str(item.get("url") or "").strip()
    ...
    if len(sources) < 4:
        proxy = _search_via_jina_ai_proxy(query=query, max_results=max_results)
        if proxy:
            seen = {str(s.get("url") or "").strip() for s in sources}
            for item in proxy:
                key = str(item.get("url") or "").strip()
    ...
    if len(sources) < 4:
        shortcuts = _build_search_shortcuts(query)
        seen = {str(s.get("url") or "").strip().lower() for s in sources}
...
def download_installer():
    """Return a portable installer ZIP so users can run the app locally on Windows."""
...
color 0A
echo =====================================================
echo   Portable OCR Studio  -  Windows Installer
echo =====================================================
echo.

:: ---- Locate Python (skip Windows Store stub) ----
set PYTHON_EXE=
...
:: 2) Try python in PATH but reject the WindowsApps stub
if "!PYTHON_EXE!"=="" (
...
if "!PYTHON_EXE!"=="" (
    for %%V in (313 312 311 310) do (
        if "!PYTHON_EXE!"=="" (
            if exist "C:\Python%%V\python.exe" set PYTHON_EXE=C:\Python%%V\python.exe
        )
    )
)
if "!PYTHON_EXE!"=="" (
...
if "!PYTHON_EXE!"=="" (
    ...
    echo   Download and install it from: https://python.org/downloads
...
:: Save path for launch.bat to reuse
...
if "!TESS_EXE!"=="" (
    ...
    if not errorlevel 1 (
        for /f "delims=" %%T in ('where tesseract 2^>nul') do (
            if "!TESS_EXE!"=="" set TESS_EXE=%%T
        )
    )
)

if "!TESS_EXE!"=="" (
    echo [NOTICE] Tesseract OCR engine not found.
    ...
    echo [OK] Tesseract found: !TESS_EXE!
    :: Save Tesseract path so launch.bat can set TESSERACT_CMD
    echo !TESS_EXE!> .tesseract_path.txt
)
...
"""
    ...
    readme_txt = (
        "Portable OCR Studio - Local Installer\n"
        "======================================\n\n"
        "Requirements:\n"
        ...
        "Quick start:\n"
        "  1. Install Python 3.10+ and Tesseract OCR (links above)\n"
        "  2. Double-click install.bat  (only needed once)\n"
        "  3. Double-click launch.bat   (starts app + opens browser)\n\n"
        "The app will be available at http://localhost:7860\n\n"
        "Troubleshooting:\n"
        "  - 'Access Denied' on install.bat -> right-click, Run as Administrator\n"
        "  - Python not found -> reinstall Python, check 'Add to PATH'\n"
        "  - Tesseract not found -> install from link above, re-run install.bat\n"
    )

    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        # Add source files
        for fname in ("app.py", "index.html", "requirements.txt"):
            fpath = os.path.join(base_dir, fname)
            if os.path.exists(fpath):
                zf.write(fpath, f"portable-ocr-studio/{fname}")
        # Add installer / launcher scripts
        zf.writestr("portable-ocr-studio/install.bat", install_bat)
        zf.writestr("portable-ocr-studio/launch.bat", launch_bat)
        zf.writestr("portable-ocr-studio/README.txt", readme_txt)

    buf.seek(0)
    return send_file(
        buf,
        as_attachment=True,
        download_name="portable-ocr-studio-installer.zip",
        mimetype="application/zip",
    )
